...
const String gpsin = "0 0";
const String gpsout = "7 7";
int in = 0;
const String ID = "vh02";
...
void mqttLoop(String tag) {
  ...
  if (now - messageSentAt > 100) { //change the delay here to manipulate the device shadow updation speed
    ...
    if (in == 0)
    {
      pubMes = "{ \"ID\":\"" + ID + "\", \"Time\":\"" + tim + "\",\"Tag\":\"" + tag + "\", \"GPS\":\"" + gpsin + "\"}";  
      in = 1;  
    }
    else
    {
      pubMes = "{ \"ID\":\"" + ID + "\", \"Time\":\"" + tim + "\",\"Tag\":\"" + tag + "\", \"GPS\":\"" + gpsout + "\"}";    
      in = 0;
    }
